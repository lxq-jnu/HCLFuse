...
class DiffusionUNet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                skip = hs.pop()
                if skip.size(2) != h.size(2) or skip.size(3) != h.size(3):
                    skip = torch.nn.functional.interpolate(skip, size=(h.size(2), h.size(3)), mode="nearest")
        ...
        h = nonlinearity(h)
        h = self.conv_out(h)
        if h.size(2) != input_size[0] or h.size(3) != input_size[1]:
